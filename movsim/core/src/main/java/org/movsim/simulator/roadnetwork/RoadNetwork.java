...
import XmlConverter.MapConverter;
...
public class RoadNetwork implements SimulationTimeStep, Iterable<RoadSegment> {
    ...
    public void timeStep(double dt, double simulationTime, long iterationCount) {
        ...
        try {
            //externalVehicleController.publishData(this, simulationTime);
            externalVehicleController.updateSelfVehicleByTrajectory(
                    externalVehicleController.getTrajectoryFromPlanning(this, simulationTime),
                    simulationTime,
                    dt
            );
        } catch (Exception e) {
            LOG.error("Can not get trajectory...");
        }

        if (iterationCount == 50) {
            externalVehicleController.addSelfVehiclesAtBeginning(this);
            /*try {
                new MapConverter().mapToString(this);
            } catch (Exception e) {
