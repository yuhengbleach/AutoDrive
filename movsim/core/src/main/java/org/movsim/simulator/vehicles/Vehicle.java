...
public class Vehicle {
    ...
    private final static double MAX_LANE_CHANGE_TIME_S = 10;

    private final static double MIN_LANE_CHANGE_TIME_S = 3;
    ...
    /** needs to be > 0 */
    private double finiteLaneChangeTimeS;
    ...
    // dx and dy represent the angle of vehicle.
    private double theta;

    private double selfTheta;
    ...
    public Vehicle(String label, LongitudinalModelBase longitudinalModel, VehiclePrototypeConfiguration vehInput,
        ...
        // step
        assert finiteLaneChangeTimeS > 0;

        this.color = Colors.color(0, 255, 0);
    ...
    public Vehicle(double rearPosition, double speed, int lane, double length, double width) {
        ...
        inhomogeneity = new InhomogeneityAdaption();
        userData = new VehicleUserData();
        LOG.info("now vehicle index is " + nextId);
    }
    ...
    private void initialize() {
        ...
        unsetSpeedlimit();
        routeIndex = 0;
        finiteLaneChangeTimeS = MIN_LANE_CHANGE_TIME_S + (int)(Math.random() * MAX_LANE_CHANGE_TIME_S);
    ...
    public double getPositionX() {
        return getRoadSegment().roadMapping().getPosTheta(this).getScreenX();
    }

    public double getPositionY() {
        return getRoadSegment().roadMapping().getPosTheta(this).getScreenY();
    }

    public double getTheta() {
        return getRoadSegment().roadMapping().getPosTheta(this).getTheta();
    }

    public double getSelfTheta() {
        return selfTheta;
    }

    public void setSelfTheta(double selfTheta) {
        this.selfTheta = selfTheta;
    }
    ...
    public void updatePositionAndSpeed(double dt) {
        ...
        if (roadSegment.checkRoadmapping()) {
            double oldSelfTheta = selfTheta;
            selfTheta = -Math.atan2(getPositionY() - oldY, getPositionX() - oldX);
            omega = selfTheta - oldSelfTheta;
            oldX = getPositionX();
            oldY = getPositionY();
        }
    ...
    public boolean inProcessOfLaneChange() {
        return (tLaneChangeDelay > 0 && tLaneChangeDelay < finiteLaneChangeTimeS);
    }
    ...
    public double getContinousLane() {
        if (inProcessOfLaneChange()) {
            final double fractionTimeLaneChange = Math.min(1, tLaneChangeDelay / finiteLaneChangeTimeS);
