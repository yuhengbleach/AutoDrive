# This Python file uses the following encoding: utf-8
"""autogenerated by genpy from av_msgs/RadarObjectList.msg. Do not edit."""
import sys
python3 = True if sys.hexversion > 0x03000000 else False
import genpy
import struct

import av_msgs.msg

class RadarObjectList(genpy.Message):
  _md5sum = "c5193a6df61f3d5bda7cb7e3b05fb09f"
  _type = "av_msgs/RadarObjectList"
  _has_header = False #flag to mark the presence of a Header object
  _full_text = """uint64 timestamp
string flag
RadarObject[] radar_object_list

================================================================================
MSG: av_msgs/RadarObject
uint64 id               # 物体ID

float32 range           # 物体的距离 0 到 204.7 m
float32 angle           # 物体的方向角 -51.2 to 51.1 deg
float32 velocity        # 物体的速度    -81.92 to 81.91 m/s
float32 accel           # 物体的加速度  -25.60 to 25.55 m/s/s
string msg

"""
  __slots__ = ['timestamp','flag','radar_object_list']
  _slot_types = ['uint64','string','av_msgs/RadarObject[]']

  def __init__(self, *args, **kwds):
    """
    Constructor. Any message fields that are implicitly/explicitly
    set to None will be assigned a default value. The recommend
    use is keyword arguments as this is more robust to future message
    changes.  You cannot mix in-order arguments and keyword arguments.

    The available fields are:
       timestamp,flag,radar_object_list

    :param args: complete set of field values, in .msg order
    :param kwds: use keyword arguments corresponding to message field names
    to set specific fields.
    """
    if args or kwds:
      super(RadarObjectList, self).__init__(*args, **kwds)
      #message fields cannot be None, assign default values for those that are
      if self.timestamp is None:
        self.timestamp = 0
      if self.flag is None:
        self.flag = ''
      if self.radar_object_list is None:
        self.radar_object_list = []
    else:
      self.timestamp = 0
      self.flag = ''
      self.radar_object_list = []

  def _get_types(self):
    """
    internal API method
    """
    return self._slot_types

  def serialize(self, buff):
    """
    serialize message into buffer
    :param buff: buffer, ``StringIO``
    """
    try:
      buff.write(_struct_Q.pack(self.timestamp))
      _x = self.flag
      length = len(_x)
      if python3 or type(_x) == unicode:
        _x = _x.encode('utf-8')
        length = len(_x)
      if python3:
        buff.write(struct.pack('<I%sB'%length, length, *_x))
      else:
        buff.write(struct.pack('<I%ss'%length, length, _x))
      length = len(self.radar_object_list)
      buff.write(_struct_I.pack(length))
      for val1 in self.radar_object_list:
        _x = val1
        buff.write(_struct_Q4f.pack(_x.id, _x.range, _x.angle, _x.velocity, _x.accel))
        _x = val1.msg
        length = len(_x)
        if python3 or type(_x) == unicode:
          _x = _x.encode('utf-8')
          length = len(_x)
        if python3:
          buff.write(struct.pack('<I%sB'%length, length, *_x))
        else:
          buff.write(struct.pack('<I%ss'%length, length, _x))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))

  def deserialize(self, str):
    """
    unpack serialized message in str into this message instance
    :param str: byte array of serialized message, ``str``
    """
    try:
      if self.radar_object_list is None:
        self.radar_object_list = None
      end = 0
      start = end
      end += 8
      (self.timestamp,) = _struct_Q.unpack(str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      start = end
      end += length
      if python3:
        self.flag = str[start:end].decode('utf-8')
      else:
        self.flag = str[start:end]
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      self.radar_object_list = []
      for i in range(0, length):
        val1 = av_msgs.msg.RadarObject()
        _x = val1
        start = end
        end += 24
        (_x.id, _x.range, _x.angle, _x.velocity, _x.accel,) = _struct_Q4f.unpack(str[start:end])
        start = end
        end += 4
        (length,) = _struct_I.unpack(str[start:end])
        start = end
        end += length
        if python3:
          val1.msg = str[start:end].decode('utf-8')
        else:
          val1.msg = str[start:end]
        self.radar_object_list.append(val1)
      return self
    except struct.error as e:
      raise genpy.DeserializationError(e) #most likely buffer underfill


  def serialize_numpy(self, buff, numpy):
    """
    serialize message with numpy array types into buffer
    :param buff: buffer, ``StringIO``
    :param numpy: numpy python module
    """
    try:
      buff.write(_struct_Q.pack(self.timestamp))
      _x = self.flag
      length = len(_x)
      if python3 or type(_x) == unicode:
        _x = _x.encode('utf-8')
        length = len(_x)
      if python3:
        buff.write(struct.pack('<I%sB'%length, length, *_x))
      else:
        buff.write(struct.pack('<I%ss'%length, length, _x))
      length = len(self.radar_object_list)
      buff.write(_struct_I.pack(length))
      for val1 in self.radar_object_list:
        _x = val1
        buff.write(_struct_Q4f.pack(_x.id, _x.range, _x.angle, _x.velocity, _x.accel))
        _x = val1.msg
        length = len(_x)
        if python3 or type(_x) == unicode:
          _x = _x.encode('utf-8')
          length = len(_x)
        if python3:
          buff.write(struct.pack('<I%sB'%length, length, *_x))
        else:
          buff.write(struct.pack('<I%ss'%length, length, _x))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))

  def deserialize_numpy(self, str, numpy):
    """
    unpack serialized message in str into this message instance using numpy for array types
    :param str: byte array of serialized message, ``str``
    :param numpy: numpy python module
    """
    try:
      if self.radar_object_list is None:
        self.radar_object_list = None
      end = 0
      start = end
      end += 8
      (self.timestamp,) = _struct_Q.unpack(str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      start = end
      end += length
      if python3:
        self.flag = str[start:end].decode('utf-8')
      else:
        self.flag = str[start:end]
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      self.radar_object_list = []
      for i in range(0, length):
        val1 = av_msgs.msg.RadarObject()
        _x = val1
        start = end
        end += 24
        (_x.id, _x.range, _x.angle, _x.velocity, _x.accel,) = _struct_Q4f.unpack(str[start:end])
        start = end
        end += 4
        (length,) = _struct_I.unpack(str[start:end])
        start = end
        end += length
        if python3:
          val1.msg = str[start:end].decode('utf-8')
        else:
          val1.msg = str[start:end]
        self.radar_object_list.append(val1)
      return self
    except struct.error as e:
      raise genpy.DeserializationError(e) #most likely buffer underfill

_struct_I = genpy.struct_I
_struct_Q = struct.Struct("<Q")
_struct_Q4f = struct.Struct("<Q4f")
