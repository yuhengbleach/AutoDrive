...
void spline::getClosestPointOnCurve(const spline& curve_x, const spline& curve_y, double x0, double y0, double& s, double& dist, int max_iteration, double converge_threshold, bool use_bisection, double bisection_threshold) {
	...
	if(use_bisection) {
		mid_s = (max_s + min_s) / 2;
		dist_min_s = computeDist(x0, y0, curve_x, curve_y, min_s);
		dist_max_s = computeDist(x0, y0, curve_x, curve_y, max_s);
		dist_mid_s = computeDist(x0, y0, curve_x, curve_y, mid_s);
		while(fabs(max_s - min_s) > bisection_threshold) {
			if(dist_min_s > dist_max_s) {
				min_s = mid_s;
				dist_min_s = dist_mid_s;
			} else {
				max_s = mid_s;
				dist_max_s = dist_mid_s;
			}
			mid_s = (min_s + max_s) / 2;
			dist_mid_s = computeDist(x0, y0, curve_x, curve_y, mid_s);
		}
